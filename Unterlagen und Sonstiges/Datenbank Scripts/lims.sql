...
CREATE TABLE IF NOT EXISTS `experiment` (
  ...
  CONSTRAINT `experiment_probe_proben_nr_fk` FOREIGN KEY (`proben_nr`) REFERENCES `probe` (`proben_nr`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
DROP TABLE IF EXISTS `experimente_analysen`;
CREATE TABLE IF NOT EXISTS `experimente_analysen` (
  ...
  CONSTRAINT `experimente_analysen_analysen_id_fk` FOREIGN KEY (`analyse`) REFERENCES `analysen` (`id`) ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `experimenttyp` (
  ...
  CONSTRAINT `experimenttyp_experiment_id_fk` FOREIGN KEY (`id`) REFERENCES `experiment` (`id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
