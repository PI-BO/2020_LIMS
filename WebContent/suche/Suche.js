...
export default (function () {
	...
	// URL
	const servletURL = Address.SUCHE.SERVLET;
	const pageURL = Address.SUCHE.JSP;

	Suche.htmlElement = {};

	Suche.render = async function render(htmlElementId) {

		Suche.htmlElement = document.getElementById(htmlElementId);
	...
	Suche.addSearchLinkToInputWithName = function addSearchLinkToInputWithName(inputElementName, templateParameters, returnParameter = new Parameter("", displayKey), linkText = "suchen", optionalCallbackBeginning, optionalCallbackEnd) {
		...
		function addListener(searchLink, templateParameters, inputElement, returnParameter, optionalCallbackBeginning, optionalCallbackEnd) {
			...
			searchLink.addEventListener("click", () => {
				...
				Suche.initTemplateParameters(templateParameters);
				Suche.resetPositionIfOutOfBounds();
				Suche.addSearchCallback((callbackContent) => {
	...
	Suche.initTemplateParameters = function initTemplateParameters(templateParameters) {
		...
		function addSelectOptions(category, selectElement) {
			...
			if (searchParameters === undefined) {
				alert("Noch kein Eintrag vorhanden (Suche.addSelectOptions)");
				return;
			}
	...
	function createParameters(event) {
		...
		const selectElement = row.getElementsByClassName(searchParameterClass)[0];
	...
	function renderResultTable(results, resultTableId) {
		...
		addTupelAsTableHeader(parameters, resultTableId, secondHeaderClass, false, { addSortFunction: true });
	...
	function addResultsToTable(results, resultTableId) {
		addTupelArrayToResults(results, resultTableId, displayKey, true);
	}
		...
		results.forEach(tupel => {
			...
			function addShowDetailsListener(cell, tupelElement) {
				...
				cell.addEventListener("mouseenter", () => {
					if (Suche.hasCallbackMethod()) return;
					if (searchCallbackForInputMasks !== undefined) return;
					cell.className = resultCellClass;
				});
				...
				cell.addEventListener("click", () => {
					...
					addTupelArrayToResults([[tupelElement]], resultTableId, undefined, false);
				});
			}
		})
	}
